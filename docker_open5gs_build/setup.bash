...
function RUNNING() {
	echo -e "${BRed}Running :"
	echo -e "${BCyan}$(cat COMMAND)${Reset}"
	bash ./COMMAND
	rm -f COMMAND
}
...
for ID in $MSISDN ; do
	...
	if [[ "$COUNT" != "1" ]]; then
		...
		# THIS WAS A BIG ERROR # echo "curl -X 'GET' 'http://$DOCKER_IP:8080/auc/imsi/$imsi' -H 'accept: application/json' | json_pp | grep auc_id | sed 's#[ \"auc_id,:]*##g'" > COMMAND
		auc_id=$(curl -X 'GET' "http://$DOCKER_IP:8080/auc/imsi/$imsi" -H 'accept: application/json' | json_pp | grep auc_id | sed 's#[ \"auc_id,:]*##g')
		
		echo -e "${BYellow}=> subscriber $msisdn auc_id $auc_id <=${Reset}"
		...
		echo "curl -X 'PUT' 'http://$DOCKER_IP:8080/ims_subscriber/' -H 'accept: application/json' -H 'Content-Type: application/json' -d '{ \"imsi\": \"$imsi\", \"msisdn\": \"$msisdn\", \"sh_profile\": \"string\", \"scscf_peer\": \"scscf.ims.mnc${MNC}.mcc${MCC}.3gppnetwork.org\", \"msisdn_list\": \"[${msisdn}]\", \"ifc_path\": \"default_ifc.xml\", \"scscf\": \"sip:scscf.ims.mnc${MNC}.mcc${MCC}.3gppnetwork.org:6060\", \"scscf_realm\": \"ims.mnc${MNC}.mcc${MCC}.3gppnetwork.org\" }'" > COMMAND;RUNNING
	fi
done
